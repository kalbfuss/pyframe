...
def check_param(name, value, recurse=False, exists=True, is_int=False, is_color=False, gr=None, ge=None, lo=None, le=None, options=None):
    ...
    :type recurse: bool
    :param exists: True if parameter must have value
    :type exists: bool
    ...
    if value is None or (isinstance(value, (list,set)) and len(value) ==0):
        # Return immediately if value is optional and not defined.
        if exists is False: return
        # Ensure that value exists.
    ...
    if isinstance(value, (list,set)):
        if recurse:
            for v in value:
                check_param(name, v, False, exists, is_int, is_color, gr, ge, lo, le, options)
        # Prevent all further tests.
        return
